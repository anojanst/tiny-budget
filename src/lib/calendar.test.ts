...
const income = (over: Partial<IncomeStream> = {}): IncomeStream => ({
  id: 'inc',
  name: 'Salary',
  amount: 700,
  frequency: 'weekly',
...
describe('buildCashflowDays — paydays', () => {
  ...
  it('does not double-count income once an anchor exists', () => {
    ...
    const dated = buildCashflowDays({
      ...args,
      incomes: [income({ nextDue: toDateInputValue(day(3)) })],
    });
  ...
  it('keeps a monthly payday on the same day of the month', () => {
    const days = buildCashflowDays({
      ...
      startingBalance: 0,
      incomes: [income({ amount: 3000, frequency: 'monthly', nextDue: toDateInputValue(new Date(2026, 0, 20)) })],
      expenses: [],
      oneOffs: [],
    });
...
describe('buildCashflowDays — shortfalls', () => {
  it('flags the days a balance goes negative', () => {
    ...
    const days = buildCashflowDays({
      ...
      through: day(10),
      startingBalance: 500,
      incomes: [income({ amount: 800, frequency: 'weekly', nextDue: toDateInputValue(day(6)) })],
      expenses: [bill],
      oneOffs: [],
    });
...
describe('buildCashflowDays — one-off money in', () => {
  ...
  it('adds a windfall on its date without calling it a payday', () => {
    ...
    expect(days[14].credits).toEqual([
      { id: 'i1', name: 'Tax refund', amount: 1200, oneOff: true },
    ]);
