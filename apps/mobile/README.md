...
The mark is the name, set in black on yellow: "MA" for the icon, the wordmark
in full for the splash. Every image is generated by
`scripts/make-brand-assets.py` rather than drawn, so the yellow lives in one
line of Python instead of baked into a folder of PNGs nobody can edit. The
Android foreground is sized to the centre 66% the launcher guarantees, since
everything outside that is croppable decoration.
